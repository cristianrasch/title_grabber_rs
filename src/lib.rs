...
impl<'a> TitleGrabber<'a> {
    ...
    fn parse_end_url_from(&self, doc: &Html, http_client: &Arc<reqwest::Client>) -> Option<String> {
        ...
        let tweet_urls = tweet_urls.into_iter().filter_map(|url| {
            let mut ret = Some(url.to_string());
        ...
        let tweet_urls = tweet_urls.filter_map(|url| {
            ...
            if let Some(host) = url.host_str() {
                if host == TWITTER_HOST {
                    let fwd_slash_cnt = url.path().chars().filter(|&c| c == '/').count();
        ...
        if !tweet_urls.is_empty() {
            let _opt = ret.replace(itertools::join(tweet_urls.into_iter(), CSV_FIELD_SEP));
        }
        ...
        if let Some(resp) = self.get(&url, &http_client) {
            ...
            if let Some(mut resp) = res.ok() {
                if let Some(html) = resp.text().ok() {
                    ...
                    if let Some(e_url) = self.parse_end_url_from(&doc, &http_client) {
                        end_url = e_url;
                    }
